$content = Get-Content "classifieds.html" -Raw
...
$newJs = @'
      ...
      async function renderGrid() {
        ...
        // Fetch from API
        try {
          const res = await (window.ZZZ ? window.ZZZ.api.fetch(\`/api/listings.php?\${params.toString()}\`) : fetch(\`/api/listings.php?\${params.toString()}\`).then(r => r.json()));
...
$content = $content -replace '(?s)// =====================================\s*// 5\. 3D GRID RENDER.*?function toggleFav\(id, btn\) \{.*?(?=// ─── QUOTA-AWARE)', $newJs

Set-Content "classifieds.html" $content
